...
import { getInsights, getAIDatasetSummary, getAIPreprocessingSummary } from '../services/api';
...
export default function InsightsPage() {
  ...
  const [aiPreprocessingSummary, setAiPreprocessingSummary] = useState(null);
  const [loadingAiDataset, setLoadingAiDataset] = useState(false);
  const [loadingAiPreprocessing, setLoadingAiPreprocessing] = useState(false);
  const [aiDatasetError, setAiDatasetError] = useState(null);
  const [aiPreprocessingError, setAiPreprocessingError] = useState(null);
  ...
  // Fetch AI Dataset Summary
  const fetchAiDatasetSummary = useCallback(async () => {
    if (!sessionId) return;
    
    setLoadingAiDataset(true);
    setAiDatasetError(null);
    
    try {
      const response = await getAIDatasetSummary(sessionId);
      setAiDatasetSummary(response.summary);
    } catch (err) {
      setAiDatasetError(err.message || 'Failed to generate AI summary');
    } finally {
      setLoadingAiDataset(false);
    }
  }, [sessionId]);

  // Fetch AI Preprocessing Summary
  const fetchAiPreprocessingSummary = useCallback(async () => {
    if (!sessionId) return;
    
    setLoadingAiPreprocessing(true);
    setAiPreprocessingError(null);
    
    try {
      const response = await getAIPreprocessingSummary(sessionId);
      setAiPreprocessingSummary(response.summary);
    } catch (err) {
      setAiPreprocessingError(err.message || 'Failed to generate AI summary');
    } finally {
      setLoadingAiPreprocessing(false);
    }
  }, [sessionId]);
  ...
  return (
    <div className="insights-page">
        ...
        <>
          ...
          <Card className="ai-summary-card">
            <div className="ai-summary-header">
              ...
              {!aiPreprocessingSummary && !loadingAiPreprocessing && (
                <Button 
                  variant="primary" 
                  size="small"
                  onClick={fetchAiPreprocessingSummary}
                  disabled={loadingAiPreprocessing}
                >
                  ✨ Generate AI Summary
                </Button>
              )}
              {aiPreprocessingSummary && (
                <Button 
                  variant="secondary" 
                  size="small"
                  onClick={fetchAiPreprocessingSummary}
            ...
            {loadingAiPreprocessing && (
              <div className="ai-summary-loading">
                <Loader text="AI is analyzing your preprocessing steps..." />
              </div>
            )}
            
            {aiPreprocessingError && (
              <div className="ai-summary-error">
                <p>⚠️ {aiPreprocessingError}</p>
                <p className="error-hint">Make sure GEMINI_API_KEY is configured in the backend.</p>
              </div>
            )}
            ...
            {!aiPreprocessingSummary && !loadingAiPreprocessing && !aiPreprocessingError && (
              <p className="ai-summary-placeholder">
                Click "Generate AI Summary" to get an intelligent explanation of all preprocessing steps applied to your data.
              </p>
            )}
          </Card>
      ...
      {activeTab === 'insights' && (
        <>
          {/* AI Dataset Summary */}
          <Card className="ai-summary-card ai-dataset-summary">
            <div className="ai-summary-header">
              <h2>🤖 AI Executive Summary</h2>
              {!aiDatasetSummary && !loadingAiDataset && (
                <Button 
                  variant="primary" 
                  size="small"
                  onClick={fetchAiDatasetSummary}
                  disabled={loadingAiDataset}
                >
                  ✨ Generate AI Summary
                </Button>
              )}
            ...
            {loadingAiDataset && (
              <div className="ai-summary-loading">
                <Loader text="AI is analyzing your dataset..." />
              </div>
            )}
            
            {aiDatasetError && (
              <div className="ai-summary-error">
                <p>⚠️ {aiDatasetError}</p>
                <p className="error-hint">Make sure GEMINI_API_KEY is configured in the backend.</p>
              </div>
            )}
            ...
            {!aiDatasetSummary && !loadingAiDataset && !aiDatasetError && (
              <p className="ai-summary-placeholder">
                Click "Generate AI Summary" to get an intelligent executive summary of your dataset's key characteristics and insights.
              </p>
            )}
          </Card>
